...
function global_initialize() 
{
	...
	Viskit.initialize();
	defaultDataProvider = new Viskit.ide.hwElement(new Viskit.p.ProviderGET("../examples/generator2.php"),
													0,
													"dataProvider0");
																		
	timer = new Viskit.u.Timer(5000);
	...
	$(function() {
		...
		propGrid = new Viskit.ide.PropertyGrid("property-grid");
	});
...
function dropped(event, ui) {
	...
	{
		case "piechart":
			var chart = new Viskit.s.chart.raphael.PieChart()
						...
						.showValue(true)
						.width(300);
			var uwElement = new Viskit.ide.uwElement(r, chart, 300, 300, defaultDataProvider, true, "pieChart" + uwElements.length, pieChartPropObj, 0);
		...
		case "columnchart":
			...
			break;
		case "dataprovider1":
			var dataProvider = new Viskit.ide.hwElement(new Viskit.p.ProviderGET("../examples/generator2.php"),
									0,
...
function generateCode() {
	...
	code += "<html>\n";
	code += "	<head>\n";
	code += " 		<title>Viskit IDE Demo</title>\n";
	code += "		<script type=\"text/javascript\" src=\"Viskit.js\"></script>\n";
	code += "		<script language=\"javascript\" type=\"text/javascript\">\n";
	code += "			var r;\n";					
	for (i = 0; i < hwElements.length; i++) {
		if (hwElements[i].type == 0)
			code += "			var "+ hwElements[i].name +" = new Viskit.p.ProviderGET(\"http://localhost/Viskit/examples/generator2.php\"); \n";
	}
	code += "			var timer = new Viskit.u.Timer(5000);\n";
	for (i = 0; i < uwElements.length; i++) {
		switch (uwElements[i].type)
		{
			case 0:
				code += "			var "+ uwElements[i].name +" = new Viskit.s.chart.raphael.PieChart(); \n";
		}
	}
